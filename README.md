...
- Codex CLI를 `subprocess`로 호출 → 로그인된 ChatGPT 계정의 OAuth 토큰 사용 (기본 리뷰 순서 `gpt-5.5` → `gpt-5.3-codex-spark`)
...
> 참고: 기본 모델인 `gpt-5.5`는 Codex CLI(ChatGPT auth) 카탈로그 기준
> 입력 윈도우 272,000의 95%인 258,400을 유효 프롬프트 예산으로 사용한다.
...
| `CODEX_MODEL` | `gpt-5.5` | 1순위 리뷰 모델 |
| `CODEX_MODEL_FALLBACKS` | `gpt-5.3-codex-spark` | 쉼표로 구분한 fallback 모델 목록. 비우면 fallback 없이 `CODEX_MODEL`만 사용 |
| `CODEX_REASONING_EFFORT` | `high` | `low`/`medium`/`high`/`xhigh` |
| `CODEX_MAX_INPUT_TOKENS` | `258400` | Codex CLI에 전달할 입력 프롬프트 토큰 예산. 기본값은 1순위 5.5 모델의 유효 컨텍스트에 맞춘 값 |
