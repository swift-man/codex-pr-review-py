...
- Codex CLI를 `subprocess`로 호출 → 로그인된 ChatGPT 계정의 OAuth 토큰 사용 (기본 리뷰 순서 `gpt-5.6-sol` → `gpt-5.5` → `gpt-5.3-codex-spark`)
...
| `CODEX_MAX_INPUT_TOKENS` | `(모델 윈도우의 95%)` | Codex CLI에 전달할 입력 프롬프트 토큰 예산. 기본 Sol은 `828400`, 5.5는 `258400`, Spark는 `121600` |
