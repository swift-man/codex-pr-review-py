...
def _review_pr_and_dump() -> tuple[Any, Any]:
...
def _review_json(summary: str = "ok") -> bytes:
    return (
        b'{"summary":"'
        + summary.encode()
        + b'","event":"COMMENT","positives":[],"must_fix":[],"improvements":[],"comments":[]}'
    )
...
async def test_review_remembers_last_successful_slot_and_rotates_from_it(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[str] = []
    success_model = "slot-2"

    async def fake_create(*args: Any, **_kwargs: Any) -> _FakeProc:
        nonlocal success_model
        argv = list(args)
        model = argv[argv.index("--model") + 1]
        calls.append(model)
        if model == success_model:
            return _FakeProc(0, stdout=_review_json(model))
        return _FakeProc(1, stderr=f"{model} reached usage limit\n".encode())
    ...
    pr, dump = _review_pr_and_dump()
    eng = CodexCliEngine(
        binary="codex",
        model="slot-1",
        model_slots=("slot-1", "slot-2", "slot-3"),
    )

    first = await eng.review(pr, dump)
    assert first.summary == "slot-2"
    assert calls == ["slot-1", "slot-2"]

    calls.clear()
    success_model = "slot-1"
    second = await eng.review(pr, dump)
    assert second.summary == "slot-1"
    assert calls == ["slot-2", "slot-3", "slot-1"]


async def test_review_raises_last_slot_error_when_all_slots_fail(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from codex_review.interfaces import ReviewEngineError
    ...
    async def fake_create(*args: Any, **_kwargs: Any) -> _FakeProc:
        ...
        model = argv[argv.index("--model") + 1]
        calls.append(model)
        return _FakeProc(1, stderr=f"{model} usage limit\n".encode())
    ...
    assert calls == ["slot-1", "slot-2", "slot-3"]
    assert "model=slot-3" in str(exc_info.value)
